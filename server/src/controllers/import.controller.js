...
// Maps flexible column headers to our internal field names.
const HEADER_MAP = {
  name: 'name',
  productname: 'name',
  itemname: 'name',
...
async function importItems(req, res) {
  ...
  rawRows.forEach((rawRow, idx) => {
    ...
    const retailPrice = toNumber(row.retailPrice, 0);
    const wholesalePrice = toNumber(row.wholesalePrice, 0);
    const reorderLevel = toNumber(row.reorderLevel, 0);

    if ([quantity, costPrice, retailPrice, wholesalePrice, reorderLevel].some((n) => Number.isNaN(n))) {
